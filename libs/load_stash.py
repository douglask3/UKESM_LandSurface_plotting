...
def coordRangeExtract(dat, lon = None, lat = None, point = None, point_as_ij = False):
...
class load_stash(object):
    ...
    def __init__(self, files, code, lbelvs, soillvs, name, units = None,
        ...
        if self.dat is not None:
            ...
            if isinstance(self.self, list):     
                self.dat = [coordRange(i, lon, lat, point, point_as_ij) for i in self.dat]
            else:
                self.dat = coordRange(dat, lon, lat, point, point_as_ij)
